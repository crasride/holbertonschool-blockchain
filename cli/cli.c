...
*/
int find_command(char *cmd, char *arg1, char *arg2, EC_KEY **ec_key, char *receiver_address)
{
	...
	else if (strcmp(cmd, "send") == 0)
	{
		...
		else
		{
			...
			return (handle_send(amount, receiver_address, *ec_key));
		}
	}
	else
	{
...
int handle_send(int amount, const char *receiver_address, EC_KEY *sender_key)
{
	...
	tx = transaction_create(sender_key, receiver_key, amount, NULL);
	if (!tx)
	{
	...
	if (!transaction_is_valid(tx, NULL))
	{
	...
	if (llist_add_node(transaction_pool, tx, ADD_NODE_REAR) == -1)
	{
...
int main(void)
{
	...
	size_t len;
	int ret = 0;
	char *line = NULL, *cmd, *arg1, *arg2, receiver_address[ADDRESS_MAX_LEN];;
	...
	while (1)
	{
		...
		ret = find_command(cmd, arg1, arg2, &ec_key, receiver_address);
		if (ret == 1)
			break;
	}
	if (line)
